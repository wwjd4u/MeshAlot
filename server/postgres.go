...
// PostgresStore is single-owner POC storage, not a multi-tenant authorization model.
...
func OpenPostgres(ctx context.Context, dsn, owner string) (*PostgresStore, error) {
	...
	if err != nil { return nil, errors.New("invalid database configuration") }
	...
	// Check the required schema through runtime privileges, not metadata grants.
	...
	if err != nil { db.Close(); return nil, errors.New("node persistence schema unavailable") }
	rows.Close()
...
func (p *PostgresStore) Enroll(ctx context.Context, nodeID, version string) error {
	...
	if err != nil { return err }
	// Compare UUIDs in PostgreSQL to avoid casing/format differences.
	var owned bool
	if err=tx.QueryRowContext(ctx,"SELECT $1::uuid=$2::uuid",owner,p.owner).Scan(&owned); err!=nil { return err }
	if !owned { return ErrNodeOwnership }
	if _,err=tx.ExecContext(ctx,"UPDATE nodes SET agent_version=$2 WHERE id=$1::uuid",id,version); err!=nil { return err }
...
func (p *PostgresStore) Heartbeat(ctx context.Context, nodeID, mode string) error {
	// Server time is authoritative. Re-enrollment never erases the last heartbeat.
...
func (p *PostgresStore) Nodes(ctx context.Context) ([]protocol.Node,error) {
	rows,err:=p.db.QueryContext(ctx,`SELECT n.node_key,n.agent_version,s.status,s.mode,s.last_heartbeat
        FROM nodes n JOIN node_status s ON s.node_id=n.id WHERE n.user_id=$1::uuid ORDER BY n.node_key`,p.owner)
	...
	for rows.Next() {
		...
		if err=rows.Scan(&n.NodeID,&n.AgentVersion,&n.Status,&n.Mode,&heartbeat);err!=nil {return nil,fmt.Errorf("scan node: %w",err)}
		if heartbeat.Valid {n.LastHeartbeat=heartbeat.Time.UTC()}
